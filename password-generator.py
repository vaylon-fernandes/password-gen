...
class PasswordGen:
    ...
    def __init__(self, base):
        ...
        self.lbl_lowercase = Label(
            self.frame, text='Do you want lowercase letters (a-z) ?:')
        self.lbl_lowercase.grid(column=0, row=1)

        self.lower_yes = Radiobutton(
            self.frame, text='Yes', variable=self.low, value=1)
        self.lower_yes.grid(column=1, row=1)

        self.lower_no = Radiobutton(
            self.frame, text='No', variable=self.low, value=0)
        self.lower_no.grid(column=2, row=1)

        self.lbl_uppercase = Label(
            self.frame, text='Do you want uppercase letters (A-Z) ?:')
        self.lbl_uppercase.grid(column=0, row=2)

        self.upper_yes = Radiobutton(
            self.frame, text='Yes', variable=self.upper, value=1)
        self.upper_yes.grid(column=1, row=2)

        self.upper_no = Radiobutton(
            self.frame, text='No', variable=self.upper, value=0)
        self.upper_no.grid(column=2, row=2, sticky='e')

        self.lbl_digits = Label(
            self.frame, text='Do you want numbers (0-9) ?:')
        self.lbl_digits.grid(column=0, row=3)

        self.digits_yes = Radiobutton(
            self.frame, text='Yes', variable=self.digit, value=1)
        self.digits_yes.grid(column=1, row=3)

        self.digits_no = Radiobutton(
            self.frame, text='No', variable=self.digit, value=0)
        self.digits_no.grid(column=2, row=3, sticky='e')

        self.lbl_punct = Label(
            self.frame, text='Do you want punctuation marks \n(eg:!\"#$%&\'()*+,-.) ?:')
        self.lbl_punct.grid(column=0, row=4)

        self.punct_yes = Radiobutton(
            self.frame, text='Yes', variable=self.punct, value=1)
        self.punct_yes.grid(column=1, row=4)

        self.punct_no = Radiobutton(
            self.frame, text='No', variable=self.punct, value=0)
        ...
        self.inputbox_length = Entry(
            self.frame, width=10, validate="key", validatecommand=(vcmd, '%P'))
        ...
        self.display_password = Listbox(
            self.mid_frame, height=1, width=30, xscrollcommand=self.scroll_bar.set)
        self.display_password.pack(side='''left''')
        self.scroll_bar.config(command=self.display_password.xview)

        self.generate_btn = Button(
            self.mid_frame, text="Generate", command=self.display)
        ...
        self.copy_btn = Button(
            self.bottom_frame, text="Copy to Clipboard", command=self.copy)
        self.copy_btn.pack()

        self.exit_btn = Button(
            self.bottom_frame, text='exit', command=base.destroy)
        self.exit_btn.pack()
    ...
    def get_choice(self):
        choice_str = ''
        choice_list = [self.low.get(), self.upper.get(),
                       self.digit.get(), self.punct.get()]
    ...
    def generate(self):
        ...
        if length == '0':
            ...
            try:
                ...
                messagebox.showinfo(
                    "Enter Length", "Please enter a number in the length field.")
            except IndexError:
                messagebox.showinfo("No option selected",
                                    "Please select at least one option 'Yes'.")
        return password
    ...
    def copy(self):
        ...
        if password == '':
            messagebox.showinfo("Password not generated.",
                                "Please Generate a password first.")
        else:
            pyperclip.copy(password)
            messagebox.showinfo(
                title='Info', message='Password has been copied to your Clipboard')
